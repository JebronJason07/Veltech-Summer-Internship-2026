...
def interactive_blur_comparison(image_path):
    """Interactive tool to compare blurring methods with adjustable parameters"""
    ...
    cv2.createTrackbar("Kernel Size", "Blur Comparison", 5, 31, lambda x: None)
    cv2.createTrackbar("Method", "Blur Comparison", 0, 3, lambda x: None)
    cv2.createTrackbar("Sigma/Bilateral", "Blur Comparison", 50, 150, lambda x: None)
    ...
    while True:
        ...
        # Get trackbar values
        ksize = cv2.getTrackbarPos("Kernel Size", "Blur Comparison")
        method = cv2.getTrackbarPos("Method", "Blur Comparison")
        sigma = cv2.getTrackbarPos("Sigma/Bilateral", "Blur Comparison")
        ...
        if method == 0:  # Averaging
            ...
            blurred = cv2.GaussianBlur(gray, (ksize, ksize), sigma_val)
            param_text = f"Kernel: {ksize}x{ksize}, Sigma: {sigma_val:.1f}"
        ...
        comparison = np.hstack([
            cv2.resize(cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR), (300, 250)),
            cv2.resize(display, (300, 250))
        ])

        cv2.imshow("Blur Comparison", comparison)
