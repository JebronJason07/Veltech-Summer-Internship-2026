...
# Complete guide to image thresholding
...
for block in block_sizes:
    for C in constant_C:
        ...
        test_image, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
        cv2.THRESH_BINARY, block, C
        )
    cv2.putText(adaptive_mean, f"Mean C: block={block}, C={C}", (10,30),
...
for block in block_sizes:
    for C in constant_C:
        ...
        test_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
        cv2.THRESH_BINARY, block, C
        )
...
_, manual_result = cv2.threshold(gray, manual_threshold, 255,
cv2.THRESH_BINARY)
# Create comparison
...
cv2.putText(comparison, "Original", (80, 30), cv2.FONT_HERSHEY_SIMPLEX,
0.6, (255, 255, 255), 2)
...
for i, (name, func) in enumerate(threshold_types.items()):
    ...
    cv2.putText(result_resized, name[:15], (5, 20),
...
print("-" * 50)
# Create a simulated coins image (or use real one if available)
...
# Show results
coins_noisy_bgr = cv2.cvtColor(coins_noisy, cv2.COLOR_GRAY2BGR)
coin_threshold_bgr = cv2.cvtColor(coin_threshold, cv2.COLOR_GRAY2BGR)
